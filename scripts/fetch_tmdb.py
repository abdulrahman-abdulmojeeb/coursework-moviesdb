...
def get_movies_count(conn) -> Dict:
    ...
    cursor.execute("SELECT COUNT(*) FROM movie WHERE tmdb_id IS NOT NULL")
    total_with_tmdb = cursor.fetchone()[0]

    cursor.execute("SELECT COUNT(*) FROM movie_detail")
    with_details = cursor.fetchone()[0]

    cursor.execute("SELECT COUNT(*) FROM movie_detail WHERE poster_path IS NOT NULL")
    with_poster = cursor.fetchone()[0]

    cursor.execute("SELECT COUNT(*) FROM movie_detail WHERE overview IS NOT NULL")
    with_overview = cursor.fetchone()[0]

    cursor.execute("SELECT COUNT(*) FROM movie_detail WHERE director IS NOT NULL")
