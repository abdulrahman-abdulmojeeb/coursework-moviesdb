...
def load_movies(conn, data_dir: str) -> None:
    """Load movies and genres from movie.csv."""
    ...
    execute_values(
        cursor,
        """
        INSERT INTO movie (movie_id, title, release_year, imdb_id, tmdb_id)
        VALUES %s
...
def load_ratings(conn, data_dir: str) -> None:
    ...
    execute_values(
        cursor,
        "INSERT INTO ml_user (user_id) VALUES %s ON CONFLICT DO NOTHING",
        [(u,) for u in users]
    )
    conn.commit()
...
def load_tags(conn, data_dir: str) -> None:
    ...
    execute_values(
        cursor,
        """
        INSERT INTO tag (user_id, movie_id, tag_text, timestamp)
        VALUES %s
        """,
        tags_data
    )
...
def load_links(conn, data_dir: str) -> None:
    ...
    # Update movie table with IMDB and TMDB IDs
    for movie_id, imdb_id, tmdb_id in links_data:
        cursor.execute(
            """
            UPDATE movie SET imdb_id = %s, tmdb_id = %s
            WHERE movie_id = %s
            """,
...
def load_personality(conn, data_dir: str) -> None:
    ...
    with open(personality_path, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            cursor.execute("SELECT 1 FROM ml_user WHERE user_id = %s", (user_id,))
            if cursor.fetchone():
...
def main():
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(*) FROM ml_user")
